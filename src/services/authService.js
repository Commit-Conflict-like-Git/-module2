...
export const signupWithDetail = async (formData) => {
    try {
        ...
        if (formData.role === 'trainer') {
            detailData.certificationFileName = fileName;
    ...
    try {
        ...
        if (userDoc.exists()) {
            ...
            if (userData.role === 'trainer') {
                const trainerDoc = await getDoc(doc(db, "users", user.uid, "trainer", "info_details"));
                if (trainerDoc.exists() && trainerDoc.data().approvalStatus === 'pending') {
                    await signOut(auth);
